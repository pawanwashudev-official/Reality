...
class FeatureManager(private val context: Context) {
    ...
    fun isRealityEliteEnabled(): Boolean = prefs.getBoolean("feature_reality_elite", false)
    fun setRealityEliteEnabled(enabled: Boolean) = prefs.edit().putBoolean("feature_reality_elite", enabled).apply()
    ...
    fun getRealityEliteStartTime(): Long {
        val userEmail = com.neubofy.reality.google.GoogleAuthManager.getUserEmail(context) ?: return 0L
        val userId = com.neubofy.reality.utils.IdentityManager.getUserId(context)
        return prefs.getLong("feature_reality_elite_start_time_$userId", 0L)
    }

    fun setRealityEliteStartTime(timeMs: Long) {
        val userEmail = com.neubofy.reality.google.GoogleAuthManager.getUserEmail(context) ?: return
        val userId = com.neubofy.reality.utils.IdentityManager.getUserId(context)
        val currentStart = prefs.getLong("feature_reality_elite_start_time_$userId", 0L)
        if (currentStart == 0L) {
            prefs.edit().putLong("feature_reality_elite_start_time_$userId", timeMs).apply()
        }
    }

    fun getRealityEliteEndTime(): Long {
        val userEmail = com.neubofy.reality.google.GoogleAuthManager.getUserEmail(context) ?: return 0L
        val userId = com.neubofy.reality.utils.IdentityManager.getUserId(context)
        return prefs.getLong("feature_reality_elite_verified_until_$userId", 0L)
    }

    fun isRealityEliteVerified(): Boolean {
        val userEmail = com.neubofy.reality.google.GoogleAuthManager.getUserEmail(context) ?: return false
        val userId = com.neubofy.reality.utils.IdentityManager.getUserId(context)

        // Check for 1-year timestamp
        val verifiedUntil = prefs.getLong("feature_reality_elite_verified_until_$userId", 0L)

        // Backwards compatibility for old boolean verified users (grace period)
        val legacyVerified = prefs.getBoolean("feature_reality_elite_verified_$userId", false)
        if (legacyVerified) {
            // Migrate them to 1 year from now
            setRealityEliteVerified(true)
            prefs.edit().remove("feature_reality_elite_verified_$userId").apply()
            return true
        }
        ...
        if (!isValid && verifiedUntil > 0) {
            ...
            setRealityEliteVerified(false)
            val proPrefs = com.neubofy.reality.utils.SecurePreferences.get(context, "reality_elite_prefs")
            proPrefs.edit().remove("pro_saved_verification_code_for_$userId").apply()
            prefs.edit().remove("feature_reality_elite_start_time_$userId").apply()
        }
        return isValid
    }

    fun setRealityEliteVerified(verified: Boolean, currentTimeMs: Long = System.currentTimeMillis(), months: Int = 12) {
        val userEmail = com.neubofy.reality.google.GoogleAuthManager.getUserEmail(context) ?: return
        val userId = com.neubofy.reality.utils.IdentityManager.getUserId(context)
        if (verified) {
            val durationMs = (365L / 12) * months * 24 * 60 * 60 * 1000
            val verifiedUntil = currentTimeMs + durationMs
            prefs.edit().putLong("feature_reality_elite_verified_until_$userId", verifiedUntil).apply()
        } else {
            prefs.edit().remove("feature_reality_elite_verified_until_$userId").apply()
            prefs.edit().remove("feature_reality_elite_verified_$userId").apply()
        }
    }
