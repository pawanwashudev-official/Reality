...
class StatisticsActivity : AppCompatActivity() {
    ...
    private fun loadTodayAppUsage() {
        ...
        // Use Aggregate functions for better accuracy over a range
        ...
        statsMap.forEach { (pkg, usageStats) ->
            if (pkg == "com.android.systemui" || pkg.contains("launcher")) return@forEach
            
            if (usageStats.totalTimeInForeground > 0) {
                totalToday += usageStats.totalTimeInForeground
                
                // Only showing apps with > 1 min usage
                if (usageStats.totalTimeInForeground > 60000) {
                     try {
                         val appInfo = packageManager.getApplicationInfo(pkg, 0)
                         val label = packageManager.getApplicationLabel(appInfo).toString()
                         val icon = packageManager.getApplicationIcon(appInfo)
                         appUsageList.add(AppUsageItem(pkg, label, usageStats.totalTimeInForeground, icon))
                     } catch (e: Exception) {
                         // System app or uninstalled
                     }
                }
            }
        }
        
        // Update Total Text - based on aggregated stats which is more accurate
        val hours = TimeUnit.MILLISECONDS.toHours(totalToday)
        val mins = TimeUnit.MILLISECONDS.toMinutes(totalToday) % 60
